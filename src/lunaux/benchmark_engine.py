    ...
    def execute(
        ...
        if process.returncode != 0:
            return BackendExecution(
                ...
                stderr=stderr,
                return_code=process.returncode,
                error=f"external command exited with code {process.returncode}",
                peak_memory_bytes=peak[0],
            )
        ...
        try:
            ...
            execution = _failure(exc, "could not read or write external output")
            return BackendExecution(
                execution.status,
                execution.output,
                execution.stderr,
...
def _child_pids(pid: int) -> tuple[int, ...]:
    ...
    try:
        return tuple(int(value) for value in children.read_text().split())
    except (OSError, ValueError):
        return ()


def _rss_tree(pid: int) -> int | None:
    values: list[int] = []
    pending = [pid]
    seen: set[int] = set()
    while pending:
        ...
        if current in seen:
            continue
        seen.add(current)
        value = _process_rss_bytes(current)
...
def _monitor_process_memory(
    ...
    while not stop.is_set():
        value = _rss_tree(pid)
        if value is not None:
            peak[0] = value if peak[0] is None else max(peak[0], value)
        stop.wait(0.01)
    value = _rss_tree(pid)
    if value is not None:
        peak[0] = value if peak[0] is None else max(peak[0], value)
