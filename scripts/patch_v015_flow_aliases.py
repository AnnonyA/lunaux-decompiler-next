...
TEST_PATH = Path("tests/test_flow_types_v015.py")


def replace_once(text: str, old: str, new: str) -> str:
    if old not in text:
        raise RuntimeError(f"replacement marker not found: {old[:64]!r}")
    return text.replace(old, new, 1)


def patch_flow_analysis() -> None:
...
def main() -> None:
    patch_flow_analysis()
    write_tests()
    Path(".github/workflows/patch-v015-flow-aliases.yml").unlink(missing_ok=True)
    Path("scripts/patch_v015_flow_aliases.py").unlink(missing_ok=True)
